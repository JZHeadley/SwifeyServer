...
class DataController(val userRepository: UserRepository, val phoneRepository: PhoneRepository, val addressRepository: AddressRepository, val restaurantRepository: RestaurantRepository, val checkInRepository: CheckInRepository) {
    ...
    fun createData(): ResponseEntity<Void> {
        ...
        val user1 = User(UUID.randomUUID().toString(), "Jonathon,", "Headley", LocalDate.of(1997, Month.JULY, 23), Timestamp.from(Instant.now()), 300, phone1, null, null, null)
        val user2 = User(UUID.randomUUID().toString(), "Jonathon,", "Headley", LocalDate.of(1996, Month.AUGUST, 21), Timestamp.from(Instant.now()), 300, phone1, null, setOf(user1), null)
        val restaurant = Restaurant(null, "Potato Restaurant", "", "", address, phone2, setOf())
